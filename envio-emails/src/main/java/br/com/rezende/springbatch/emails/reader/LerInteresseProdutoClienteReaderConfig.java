package br.com.rezende.springbatch.emails.reader;
...
import br.com.rezende.springbatch.emails.model.Cliente;
...
public class LerInteresseProdutoClienteReaderConfig {
	...
	public JdbcCursorItemReader<InteresseProdutoCliente> lerInteresseProdutoClienteReader(
		...
		return new JdbcCursorItemReaderBuilder<InteresseProdutoCliente>()
				...
				.sql("select * from interesse_produto_cliente join cliente on (cliente = cliente.id) join produto on (produto = produto.id)")
				.rowMapper(rowMapper())
				.build();
	}
